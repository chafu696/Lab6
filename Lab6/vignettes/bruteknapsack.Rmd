...
In an ideal world you could speed up by a factor of as many cores as you have, though in practice it's nowhere near that due to overhead and 
For sufficiently large data frames (ie the overhead is negligible), our implementation of the parallelisation takes about 60% of the time of the single core variant. The implementation feels ugly, in that it's returning a large list with (most) elements being a list with 2 elements. This large list then is then processed to get a list of just the first elements to find the max value (this processing was over ten times faster without parallelisation). Many other methods were tried, but with the bottleneck being any parLapply() on the large lists, nothing could come close to the same speed as the final implementation.
